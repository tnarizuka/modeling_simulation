...
これらの数理モデルのうち，統計モデルと機械学習モデルは個別のデータから出発してモデルのパラメータを推定するという帰納的な手順を取り，物理モデルは現象のメカニズムを取り入れた一般的なモデルから出発して，具体的に何が起こるかを調べるという演繹的な手順を取ることが多い（ただし，データから出発して物理モデルのパラメータを推定するという分析も行われる）．
...
数理モデルの分析では，実際のデータに合うようにパラメータを調節するだけでなく，あるパラメータに対して，変数の値がどのようになるかを調べることも重要である．
これを**シミュレーション**と呼ぶ．
例えば，時間を変数として含む数理モデルでは，あるパラメータの値を固定したときに，時間の経過とともに変数の値がどのように変化するかを調べることが行われる．
