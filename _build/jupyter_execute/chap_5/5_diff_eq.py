...
# **マルサスモデル**
# 
# マルサスモデルの場合，時間刻み $ \Delta t $ を小さくするほど厳密解に近づくことが分かる．
...
# **ロジスティックモデル**
# 
# ロジスティックモデルの場合も時間刻み $ \Delta t $ を小さくすると数値解と厳密解がよく一致することが分かる．
# 一方，$ \Delta t $ を大きくしていくと，マルサスモデルとは異なった振る舞いが見られる．
# 詳しくは，演習問題を解くと分かるが，オイラー法が全く役に立たないことが実感できるだろう．
# 実は，ロジスティック方程式をオイラー法で離散化した方程式は**ロジスティック写像**と呼ばれており，**周期倍分岐**と呼ばれる特異な振る舞いをすることが知られている．
# 特に，あるパラメータの値を超えると，**カオス**と呼ばれる非常に複雑な振る舞いをすることが知られている．
