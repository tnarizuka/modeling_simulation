...
# **データの観測者からの視点**
# 
# 次に，同じ状況をデータの観測者から見ると，手元のデータ $ (x_{1},\ x_{2},\ \ldots, x_{n}) $ は，どのような母集団分布から生成されたのだろうか？という見方になる．
...
# この状況において，手元の $ n $ 個のデータが得られる同時確率は式{eq}`eq:joint_probability_1`と変わらない．
# しかし，データ $ (x_{1},\ x_{2},\ \ldots, x_{n}) $ が定数で，パラメータ $ \boldsymbol{\theta} $ が変数であることを明示するために次のように表記する：
# 
# $$
...
# 
# これを**尤度関数**と呼ぶ．
# 式{eq}`eq:joint_probability_2`（尤度関数）と式{eq}`eq:joint_probability_1`（同時確率）は，データとパラメータのどちらを変数と見るかの違いだけで，データが得られる確率を意味することは変わらない．
