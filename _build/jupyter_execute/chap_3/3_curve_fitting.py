...
# **データの生成元からの視点**
# 
# まず，データの生成元から見た場合，母集団分布 $ f(x\mid \boldsymbol{\theta}) $ のパラメータ $ \boldsymbol{\theta} $ は分かっており，毎回確率的にデータ $ x_{i} $ が生成される．
...
# **データの観測者からの視点**
# 
# 次に，同じ状況をデータの観測者から見ると，手元のデータ $ (x_{1},\ x_{2},\ \ldots, x_{n}) $ に対して，このデータの生成元となった母集団分布のパラメータ $ \boldsymbol{\theta} $ はどのような値だったのだろうか？という見方になる（母集団分布の形は既知とする）．これを確率分布のパラメータ推定と呼ぶ．この場合，手元の $ n $ 個のデータが得られる確率は母集団分布のパラメータによって変化するので，$ (x_{1},\ x_{2},\ \ldots, x_{n}) $ は定数で，パラメータ $ \boldsymbol{\theta} $ は変数と考えていることになる．
...
# 
# と表記する．これを**尤度関数**と呼ぶ．
# 尤度関数と同時確率は，データとパラメータのどちらを変数と見るかの違いだけで，中身は全く同じである．
...
# **1. データの生成**
# 
# パラメータ $ p $ のベルヌーイ分布からサイズ100の標本を発生させる．
# 例えば，$ p=0.3 $ とする．
# これが母集団分布のパラメータ（真の値）となる．
...
# **1. データの生成**
# 
# まずは平均0，標準偏差1の正規分布（標準正規分布）からサイズ100の標本を生成する．
...
# **3. 最尤推定値を求める**
# 
# 今回は `L` および `log_L` の最大値に対応する行番号と列番号を取得する必要があるので，`np.argmax`関数ではなく`np.where`関数を用いる．
...
# **4. 尤度関数を描画する**
# 
# 今回はパラメータが2つあるので，尤度関数と対数尤度をそれぞれお3次元プロットする必要がある．
