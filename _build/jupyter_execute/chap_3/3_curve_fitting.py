...
# まず，データの生成元から見た場合，確率分布のパラメータ $ \boldsymbol{\theta} $ は分かっており，毎回確率的にデータ $ x_{i} $ が生成される．
# 具体的に，$ x_{i} $ というデータが生成される確率は $ f(x_{i}|\boldsymbol{\theta}) $ であるので，$ n $ 個のデータが得られる確率は
# 
# $$
...
pd.DataFrame({'P': P, 'L': L, 'log_L': log_L}).head()
