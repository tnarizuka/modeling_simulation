...
# これまで，パラメータ $ \boldsymbol{\theta} $ を持つ１次元確率分布を $ f(x; \boldsymbol{\theta}) $ と表記していたが，変数と定数の区別を強調するために，ここからは $ f(x|\boldsymbol{\theta}) $ と表記する．
# 例えば，$ f(x|\boldsymbol{\theta}) $ と書いたときは，$ \boldsymbol{\theta} $ は与えられた定数で，$ x $ が変数を表すことになる．
...
# さて，尤度関数というのは，手元のデータが得られる確率を，生成元となった確率分布のパラメータの関数と捉えたものである．
# そこで，手元に $ n $ 個のデータが与えられたとき，尤度関数$ L(\boldsymbol{\theta}|x_{1}, x_{2},\ldots, x_{n}) $ を最大にするように生成元の確率分布 $ f(x|\boldsymbol{\theta}) $ のパラメータを決めることを考える．
...
# なお，対数尤度関数が凸関数の場合には対数尤度方程式を解くことで最尤推定値が得られるが，非凸関数の場合には最適解が一意に決まらない．
