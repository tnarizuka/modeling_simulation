...
# #### 遷移確率
# 
# 時刻 $ k $ の状態を確率変数 $ U_{k} $ によって表す．
# $ U_{k} $ は離散的な状態 $ \{0, 1, 2, \ldots\} $ を取るものとする（取りうる状態の集合を**状態空間**と呼ぶ）．
...
# $$
# 
# と表し，全状態を並べたベクトル $ \boldsymbol{w}_{k} $ を**状態ベクトル**と呼ぶ．
# 状態ベクトルは確率分布を表し，3状態の場合は $ \boldsymbol{w}_{k} = [w^{(k)}_{0}, w^{(k)}_{1}, w^{(k)}_{2}] $ と表される．
