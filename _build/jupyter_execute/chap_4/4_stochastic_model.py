...
# #### 遷移確率
# 
# これ以降，試行回数を離散的な時刻 $ k $ に読み替え，時刻 $ k $ の状態を確率変数 $ U_{k} $ によって表す．
# $ U_{k} $ は2つ以上の離散的な状態 $ \{0, 1, 2, \ldots\} $ を取るものとする（取りうる状態の集合を**状態空間**と呼ぶ）．
# このとき，マルコフ性は以下のように表される：
# 
# $$
#     P(U_{k+1}=j|U_{k}=i) = P(U_{k+1}=j|U_{k}=i, U_{k-1}, \ldots, U_{0})
# $$
# 
...
# #### 状態ベクトル
# 
# 時刻 $ k $ に各状態を取る確率をベクトル $ \boldsymbol{u}_{k} $ で表し，これを**状態ベクトル**と呼ぶ．
# 時刻 $ k+1 $ の状態ベクトルは，時刻 $ k $ の状態ベクトル $ \boldsymbol{u}_{k} $ と遷移確率行列 $ P $ を用いて
# 
# $$
#     \boldsymbol{u}_{k+1} = \boldsymbol{u}_{k}P  
# $$
# 
# と表すことができる．
...
# 
# $$
#     \boldsymbol{u}_{k} = \boldsymbol{u}_{0}P^{k}
#     
# $$
# 
# と表される．
