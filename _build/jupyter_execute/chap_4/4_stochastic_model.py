...
# これまで，試行回数を $ i $ で表していたが，これ以降は離散的な時刻 $ t $ によって表す．

# ### マルコフ連鎖の定義
# 
# ベルヌーイ過程は，１回の試行で取りうる状態が2つ（成功／失敗）だけで，それぞれの出現確率が一定値（ $ p,\ 1-p $ ）であったが，これを拡張して以下のような条件を加える：
# 
# 1. 各時刻で取りうる状態が2つ以上
# 2. 時刻 $ t+1 $ に取りうる状態が時刻 $ t $ （現在）の状態だけに依存して決まる（**マルコフ性**）
# 
# このように，状態の数が2つ以上でマルコフ性を持つ確率過程を**マルコフ連鎖**と呼ぶ．

# ### マルコフ連鎖の定式化
# 
# #### 遷移確率
# 
# 時刻 $ t $ の状態を確率変数 $ U_{t} $ によって表し，$ U_{t} $ は2つ以上の離散的な状態 $ \{0, 1, 2, \ldots\} $ を取るものとする（取りうる状態の集合を**状態空間**と呼ぶ）．
# このとき，マルコフ性は以下のように表される：
# 
# $$
#     P(U_{t+1}=j|U_{t}=i) = P(U_{t+1}=j|U_{t}=i, U_{t-1}, \ldots, U_{0})
# $$
# 
...
# 全ての状態間の遷移確率をまとめて表すには，遷移確率 $ p_{ij} $ を $ (i,j) $ 成分とする行列を考えると便利である．
# 例えば，以下は状態空間が $ S=\{0, 1, 2\} $ の場合の遷移確率行列である：
# 
# $$
#     P = \begin{pmatrix}
#         p_{00} & p_{01} & p_{02} \\
...
# $$
# 
# この例からも分かるように，遷移確率行列は各行の和が必ず1になる．
# これは，次の時刻で必ずいずれかの状態に遷移することを意味している．

# #### 状態ベクトル
# 
# 時刻 $ t $ に各状態を取る確率をベクトル $ \boldsymbol{u}_{t} $ で表し，これを**状態ベクトル**と呼ぶ．
# 時刻 $ t+1 $ の状態ベクトルは，時刻 $ t $ の状態ベクトル $ \boldsymbol{u}_{t} $ と遷移確率行列 $ P $ を用いて
# 
# $$
#     \boldsymbol{u}_{t+1} = \boldsymbol{u}_{t}P  
# $$
# 
# と表すことができる．
# これより，初期状態ベクトル $ \boldsymbol{u}_{0} $ が与えられたとき，時刻 $ t $ の状態ベクトルは
# 
# $$
#     \boldsymbol{u}_{t} = \boldsymbol{u}_{0}P^{t}
#     
# $$
# 
# と表される．
